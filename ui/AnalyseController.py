...
class Analyse_controller(QtWidgets.QMainWindow):
	...
	def _draw(self):
		x, y = [], getQuestionPoints(self.question)
	...
	def setup_control(self):
		...
		info = getQuestionInfor(self.question)
		...
		self.ui.reciteCheckBox.setCheckState(Qt.Checked if info[4] else Qt.Unchecked)
		self.ui.wrongCheckBox.setCheckState(Qt.Checked if info[5] else Qt.Unchecked)
...
# ----- 要提供的函数 ----- # TODO
def getQuestionInfor(question: str) -> (int, str, bool, bool, bool, bool, int, str):
	"""
	:param question:
	:return: 1.题目分数（如果此次记录是搜索返回-1）；2.题目类型：主观题，客观题；（没有返回''）
			...
			9.搜索题目/小练习
	"""
	return 5, '客观题', True, True, False, False, '大爹', 10, '搜索题目'


def getQuestionPoints(question: str) -> list:
	"""
	:param question:
	:return: 返回该题从app建立到现在的所有分数，不去重，没有返回空，搜索记录返回0就好 远 ---> 近
	"""
	return [3, 4, 9, 10, 6, 8, 9, 7, 8, 5, 9, 4, 6, 2]
